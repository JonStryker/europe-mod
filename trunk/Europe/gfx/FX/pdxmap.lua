...
static const float TERRAINTILEFREQ = 140.0f;
...
float3 calculate_secondary( float2 uv, float3 vColor, float2 vPos )
{
	...
	float4 vMask = tex2D( OccupationMask, vPos / 5.0f ).rgba;
	float3 vSecondColor = 
...
float4 main( VS_OUTPUT_TERRAIN Input ) : COLOR
{
	...
	float3 TerrainColor = tex2D( TerrainColorTint, Input.uv2 ).rgb;	
	sample.rgb = GetOverlay( sample.rgb, tex2D( TerrainColorTint, Input.uv2*vMapSize.zw ).rgb, 0.9f );
